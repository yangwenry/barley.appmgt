...
public class TenantWorkflowConfigHolder implements Serializable {
    ...
    public void load() throws WorkflowException {
        ...
        try {
            ...
            Resource resource = registry.get(AppMConstants.WORKFLOW_EXECUTOR_LOCATION);

            in = resource.getContentStream();
            StAXOMBuilder builder = new StAXOMBuilder(in);
            secretResolver = SecretResolverFactory.create(builder.getDocumentElement(), true);

            OMElement workflowExtensionsElem = builder.getDocumentElement();
            ...
            workflowElem = workflowExtensionsElem.getFirstChildWithName(
                    new QName(WorkflowConstants.PUBLISH_APP));
            executorClass = workflowElem.getAttributeValue(new QName("executor"));
            clazz = WorkflowExecutorFactory.class.getClassLoader().loadClass(executorClass);
            workFlowExecutor = (WorkflowExecutor)clazz.newInstance();
            loadProperties(workflowElem, workFlowExecutor);
            workflowExecutorMap.put(WorkflowConstants.WF_TYPE_AM_APP_PUBLISH, workFlowExecutor);

        } catch (XMLStreamException e) {
            log.error("Error building xml from file " + AppMConstants.WORKFLOW_EXECUTOR_LOCATION, e);
            handleException("Error building xml from file " + AppMConstants.WORKFLOW_EXECUTOR_LOCATION, e);
            ...
            handleException("Unable to Access Registry", e);
        } finally {
    ...
    public void setInstanceProperty(String name, Object val, Object obj) throws WorkflowException {
        ...
        try {
            ...
            handleException("Error invoking setter method named : " + mName +
                            "() Illegal argument exception", e);
        } catch (NoSuchMethodException e) {
            handleException("Error invoking setter method named : " + mName +
                            "() No such method found exception", e);
        } catch (SecurityException e) {
            handleException("Error invoking setter method named : " + mName +
                            "() Security exception", e);
        } catch (InvocationTargetException e) {
            handleException("Error invoking setter method named : " + mName +
                            "() Target invocation exception", e);
        } catch (WorkflowException e) {
            handleException("Error invoking setter method named : " + mName +
                            "() Exception in workflow", e);
        }
    }
