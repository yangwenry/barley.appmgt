...
public class SubscriptionCreationWSWorkflowExecutor extends WorkflowExecutor{
    ...
    public void execute(WorkflowDTO workflowDTO) throws WorkflowException{
        ...
        try {
            ...
            String subscriptionType = appMDAO.getSubscriptionType(Integer.parseInt(workflowDTO.getWorkflowReference()));
            if(subscriptionType.equals("ENTERPRISE")){
            ...
            log.error("Error retrieving from  database", e);
            throw new WorkflowException("Error retrieving from  database", e);
        }
    }
         ...
         try {
             ...
             if (subscriptionType.equals("ENTERPRISE"))   {
                 ...
                 if(WorkflowStatus.APPROVED.equals(workflowDTO.getStatus())){
                     ...
                     appMDAO.updateSubscriptionStatus(Integer.parseInt(workflowDTO.getWorkflowReference()),
                             AppMConstants.SubscriptionStatus.REJECTED);
                 }
             }
         } catch (AppManagementException e){
             log.error("Could not retrieve from database  ", e);
             throw new WorkflowException("Could not retrieve from database", e);
         }
         }
