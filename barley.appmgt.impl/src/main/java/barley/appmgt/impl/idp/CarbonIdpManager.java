...
package barley.appmgt.impl.idp;
...
public class CarbonIdpManager implements WebAppIdPManager {
	...
	public void init(AppManagerConfiguration config) throws AppManagementException {
		...
		try {
			...
			BarleyUtils.setBasicAccessSecurityHeaders(user, pwd, idpStub._getServiceClient());
			BarleyUtils.setBasicAccessSecurityHeaders(user, pwd, appMgtStub._getServiceClient());
		} catch (AxisFault e) {
			log.error(e);
			throw new AppManagementException(e.getMessage());
		}
