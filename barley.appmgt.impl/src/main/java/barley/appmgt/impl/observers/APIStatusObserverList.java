...
public class APIStatusObserverList {
    ...
    public void init(AppManagerConfiguration config) {
        if (initialized) {
            log.warn("Attempt to reinitialize APIStatusObserverList - Skipping");
            return;
        }
        
        List<String> values = config.getProperty(AppMConstants.OBSERVER);
        if (values != null) {
    ...
    public void notifyObservers(APIStatus previous, APIStatus current, WebApp api) {
