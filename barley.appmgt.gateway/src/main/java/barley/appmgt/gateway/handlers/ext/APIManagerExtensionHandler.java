...
package barley.appmgt.gateway.handlers.ext;
...
/**
 * A simple extension handler for the APIs deployed in the WebApp gateway. This handler first
 ...
 * Following that a more WebApp specific extension sequence is looked up by using the name
 * pattern provider--api--version--[Dir]. If such an WebApp specific sequence is found, that
 * is also invoked. If no extension is found either at the global level or at the per WebApp level
...
public class APIManagerExtensionHandler extends AbstractHandler {
    ...
    private void logMessageDetails(MessageContext messageContext, String direction) {
        ...
        if (isLoginRequest) {
            ...
            if (DIRECTION_IN.equals(direction)) {
                log.debug("Inbound WebApp call from client to gateway: " + logMessage);
            } else if (DIRECTION_OUT.equals(direction)) {
                logMessage = logMessage + " , EndPointURL=" + axisMC.getProperty("ENDPOINT_PREFIX");
                log.debug("Outbound WebApp call from gateway to client: " + logMessage);
            }
        }
    }
