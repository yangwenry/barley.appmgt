...
import barley.appmgt.gateway.handlers.security.oauth.OAuthAuthenticator;
import barley.appmgt.gateway.internal.ServiceReferenceHolder;
import barley.appmgt.impl.AppMConstants;

/**
 * Authentication handler for REST APIs exposed in the WebApp gateway. This handler will
...
public class APIAuthenticationHandler extends AbstractHandler implements ManagedLifecycle {
    ...
    private void logMessageDetails(MessageContext messageContext) {
        ...
        String logMessage = "WebApp call failed reason=API_authentication_failure"; //"app-name=" + applicationName + " " + "user-name=" + endUserName;
        ...
        log.debug("Call to WebApp Gateway " + logMessage);
    }
