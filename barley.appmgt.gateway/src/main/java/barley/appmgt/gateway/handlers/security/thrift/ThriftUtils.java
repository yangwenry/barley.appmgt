...
public class ThriftUtils {
    ...
    private ThriftUtils() throws APISecurityException {
        try {
            ...
            remoteServerURL = config.getFirstProperty(AppMConstants.API_KEY_MANAGER_URL);
            remoteServerPort = remoteServerURL.split(":")[2].split("/")[0];
            //we expect in the form of : https://ip:port.. as in api-mgt.xml
            thriftServerHost=config.getFirstProperty(AppMConstants.API_KEY_MANAGER_THRIFT_SERVER_HOST);
            if(thriftServerHost==null){
                thriftServerHost = remoteServerURL.split(":")[1].split("//")[1];
            }
            remoteServerIP = remoteServerURL.split(":")[1].split("//")[1];
            String thriftPortString = config.getFirstProperty(AppMConstants.API_KEY_MANGER_THRIFT_CLIENT_PORT);
            String clientTimeOutString = config.getFirstProperty(AppMConstants.API_KEY_MANGER_CONNECTION_TIMEOUT);
